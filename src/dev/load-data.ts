import { connectClient, stopClient } from "../server/database";

async function main(){
    const client = await connectClient();

    client.collection("contests").deleteMany({});

    const data = await client.collection("contests").insertMany([
        {
    ...
    console.info("Inserted Contestes:", data.insertedCount)
